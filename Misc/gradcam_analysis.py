...
if __name__ == '__main__':
    ...
    for x, y, ids in train_loader:
        ...
        for i, pid in enumerate(ids):
            ...
            os.makedirs(f'./Data/{Analysis_Name}_Results/{pid}', exist_ok=True)
            ...
            for j in range(6):  # 6 images per patient
                ...
                cv2.imwrite(f'./Data/{Analysis_Name}_Results/{pid}/{args.method}_cam_{pid}_{j}th_label={y["REC"][i].item()}.jpg', cam_image)
                grayscale_cam_uint8 = cv2.applyColorMap(grayscale_cam_uint8, cv2.COLORMAP_JET)
                cv2.imwrite(f'./Data/{Analysis_Name}_Results/{pid}/{args.method}_cam_{pid}_{j}th_label={y["REC"][i].item()}_heatmap.jpg', grayscale_cam_uint8)
                cv2.imwrite(f'./Data/{Analysis_Name}_Results/{pid}/{args.method}_cam_{pid}_{j}th_label={y["REC"][i].item()}_rgb.jpg', rgb_img_uint8)
