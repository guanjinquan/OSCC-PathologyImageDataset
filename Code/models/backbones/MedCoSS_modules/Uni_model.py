...
from models.backbones.MedCoSS_modules.Base_module import TransformerEncoderLayer, VideoBaseEmbedding
import numpy as np
...
class Unified_Model(nn.Module):
    ...
    def _change_input_chans_2D(self, in_chans):
        if in_chans != 3:
            old_embeds = self.video_embed.embeddings
            new_embeds = nn.Conv2d(1, old_embeds.weight.shape[0], kernel_size=old_embeds.kernel_size, stride=old_embeds.stride, padding=old_embeds.padding, bias=(old_embeds.bias is not None))
            new_embeds.weight.data = old_embeds.weight.data.mean(dim=1, keepdim=True).repeat(1, in_chans, 1, 1)
            new_embeds.bias.data = old_embeds.bias.data  # weight and bias are already shared
            self.video_embed.embeddings = new_embeds

    def _loadpth_with_diffsize_3D(self, checkpoint):
        state = self.state_dict()
        pretrain = {
            k: v for k, v in checkpoint.items() 
            if k in state.keys() and \
                k != "video_embed.embeddings_st_pos_3D.spatial_pos_embed.weight"
        }
        # resize pos_embed
        pos_k = "video_embed.embeddings_st_pos_3D.spatial_pos_embed.weight"
        weight = checkpoint[pos_k].cpu().numpy()
        if weight.shape != state[pos_k].shape:  # 如果posemb的size不一样，就用ndimage.zoom函数进行插值
            ntok_new = state[pos_k].shape[0]
            posemb_zoom = ndimage.zoom(weight, (ntok_new / weight.shape[0], 1), order=0)
            pretrain[pos_k] = torch.from_numpy(posemb_zoom)
        # load weights
...
#     checkpoint = torch.load("/home/Guanjq/Work/MultiTaskMI/Code/backbones/MedCoSS_modules/vit_base_p16_224_medcoss_epoch299.pth")['model']
