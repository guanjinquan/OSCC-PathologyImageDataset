...
class NNEmbeddingEncoding(nn.Module):
    def __init__(self, dim, max_len):  # 支持1600的长度 
...
class VideoBaseEmbedding(nn.Module):
    ...
    def forward(self, data):  # 自动区分2D和3D

        if data.dim() == 4:  # [bs, c, h, w]
            bs = data.size(0)
            # print(data.size())
            x = self.embeddings(data) # b*t, dim, 14, 14
            x = x.flatten(2) 
            # (bs * time_span * t) 768 hwd -> bs t hwd time_span*channels
        ...
        elif data.dim() == 5:  # [bs, c, h, w, d]
            bs = data.size(0)
            x = self.embeddings_3D(data)  # b*t, dim, 14, 14
            x = x.flatten(2)  # .flatten(2)  # bs, 768, h*w*d
            # s: spatial dim, t : temporal dim
...
class Divide_ST_POS(nn.Module):
    def __init__(self, num_patches, max_time_len, out_dim,
        ...
        self.random_temporal_pos: Any = random_temporal_pos
